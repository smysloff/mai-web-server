...
export default class HttpServer {
  ...
  use(path, ...handlers) {
    ...
    if (
      isFunction(path)
      || (isArray(path) && path.every(isFunction))
    ) {
  ...
  handle(request, response) {
    ...
    const url = getURL(this.host, request.url, this.isSecure)

    const path = url.pathname

    const handlers = this.#stack
      .filter(layer => layer.match({ method, path }))
      .map(layer => layer.handlers)
      .flat()
